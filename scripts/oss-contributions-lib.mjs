...
export function deriveBadges(contributions, verifiedImpact) {
  ...
  return [
    ...
    badge('maintainer-signal', 'maintainer signal', 'a checked accepted_fix receipt with an exact public GitHub evidence URL', accepted.length > 0, accepted.length ? 'condition met' : 'add a checked accepted_fix receipt with an exact public GitHub evidence URL', { tier: accepted.length ? 'aurora' : 'locked', evidenceUrls: accepted.map((item) => item.evidenceUrl), filter: { scope: 'upstream', status: 'merged' } }),
